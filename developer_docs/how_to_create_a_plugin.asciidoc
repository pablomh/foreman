...
----
If you want to use Foreman's `find_resource` method as
a before_action in your plugin, you will need to extend Foreman's
...
For instance, in order to replace the Properties header in the host
overview partial at `foreman/app/views/hosts/_overview.html.erb`:

[source, ruby]
----
Deface::Override.new(:virtual_path => "hosts/_overview",
                     :name => "replace_properties_header",
                     :replace => "th",
                     :text => "<th colspan='2'>Custom Properties</th>",
                     :original => "<%= _('Properties') %>")
----
...
cron entries. Do not manually add cron entries or systemd timers - use the
`Foreman::Cron.register` API instead.
  ...
  class Engine < ::Rails::Engine
    initializer 'my_plugin.register_plugin', before: :finisher_hook do
      Foreman::Plugin.register :my_plugin do
        ...
        Foreman::Cron.register(:hourly,  'my_plugin:refresh_data')
        Foreman::Cron.register(:daily,   'my_plugin:cleanup')
        Foreman::Cron.register(:weekly,  'my_plugin:rebuild_caches')
        Foreman::Cron.register(:monthly, 'my_plugin:archive_old_data')
      end
    end
  end
end
...
Foreman supports four scheduling cadences:
...
==== Important requirements for recurring tasks

WARNING: Recurring tasks must not call `exit`, `abort`, or similar functions that
terminate the process. When a task is registered, it runs as part of a batch with
other registered tasks (e.g., `foreman-rake cron:daily` runs all daily tasks
sequentially). If your task calls `exit`, it will terminate the entire rake
process and prevent subsequent tasks from running. Instead, use `raise` to signal
errors - the cron framework will catch exceptions, log them, and continue running
remaining tasks.
...
_Requires Foreman 3.16 or higher, set `requires_foreman '>= 3.17'` in
engine.rb_
...
Starting Foreman 3.16, both UI and API controllers can order by virtual columns.
This is achieved by using the `virtual_column_scope :select_<virtual_column>, lambda {}` scope in the model.
The scope should return the virtual column in the select statement.

[source, ruby]
....
# app/models/model.rb
virtual_column_scope :select_virtual_column, lambda {
